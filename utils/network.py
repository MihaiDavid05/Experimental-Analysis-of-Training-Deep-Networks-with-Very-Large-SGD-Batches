...
class GhostBatchNorm1d(BatchNorm1d):
    ...
    def forward(self, input):
        ...
        if self.training or not self.track_running_stats:
            return F.batch_norm(
                input.view(-1, C * self.num_splits), self.running_mean, self.running_var,
                self.weight.repeat(self.num_splits), self.bias.repeat(self.num_splits),
...
class VGG13CIFARBNGHOSTCLASSIF(nn.Module):
    def __init__(self, n_classes, batch_size, splits_denom=32, init_weights=True):
        super(VGG13CIFARBNGHOSTCLASSIF, self).__init__()
        ...
        self.model = nn.Sequential(
            ...
            nn.Flatten(),
            nn.Linear(512, 512),
            GhostBatchNorm1d(512, num_splits=self.nr_splits, momentum=0.99),
            nn.ReLU(inplace=True),
            nn.Dropout(0.5),
            nn.Linear(512, self.n_classes)
        )
...
def build_network(config):
    ...
    elif config["model"] == 'vgg13ghostbnclassif':
        net = VGG13CIFARBNGHOSTCLASSIF(num_classes, config["batch_size"])
